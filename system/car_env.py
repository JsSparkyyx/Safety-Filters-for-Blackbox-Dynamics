...
class CarEnv(DirectObject):
    def __init__(self, params={}):
        ...
        self.sampled_data = {'state':[],'speed':[],'rotation':[],'position':[],'collision':[]}
        ...
        if dt >= self._step:
            ...
            if self._curr_time > 0.05:
                if self._des_vel is not None:
                    ...
                    # print(vel, self._mark_d, self._is_contact(),self._des_vel)
                    err = self._des_vel - vel
    ...
    def _update_task(self, task):
        ...
        self._update(dt=dt)
        self.sampled_data['state'].append(self._get_observation())
        self.sampled_data['speed'].append(self._get_speed())
        self.sampled_data['rotation'].append(self._steering)
        self.sampled_data['position'].append(self._vehicle_pointer.getPos())
        self.sampled_data['collision'].append(self._collision)
    ...
    def reset(self, pos=None, hpr=None, hard_reset=False, random_reset=False):
        if self._run_as_task:
            path = 'D:/Code/WashU/inDCBF/trajectories/'
            idx = 0
            while os.path.exists(os.path.join(path,str(idx))):
                idx += 1
            path = os.path.join(path,str(idx))
            os.mkdir(path)
            print(np.array(self.sampled_data['state']).shape)
            np.save(os.path.join(path,'state.npy'),np.array(self.sampled_data['state']))
            np.savetxt(os.path.join(path,'velocity.txt'),np.array([self.sampled_data['speed'],self.sampled_data['rotation']]).transpose())
            np.savetxt(os.path.join(path,'position.txt'),np.array(self.sampled_data['position']))
            np.savetxt(os.path.join(path,'collision.txt'),np.array(self.sampled_data['collision']))
            self.sampled_data = {'state':[],'speed':[],'rotation':[],'position':[],'collision':[]}
