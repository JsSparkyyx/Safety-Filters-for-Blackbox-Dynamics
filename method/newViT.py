...
class Barrier(torch.nn.Module):
    def __init__(self,
        ...
        self.eps_decent = eps_decent
        self.w_safe = w_safe
        self.w_unsafe = w_unsafe
        self.w_grad = w_grad
        self.w_decent = w_decent
    ...
    def loss_function(self,x,label,u,ode):
        ...
        b_transit = self.forward(x_transit)
        ...
        if (label == 2).sum() != 0:
            x_g = x_transit.clone().detach()
            x_g.requires_grad = True
            b = self.forward(x_g)
            d_b_transit = torch.autograd.grad(b.mean(),x_g,retain_graph=True)[0]
            with torch.no_grad():
                f, g = ode(x_transit)
            gu = torch.einsum('btha,bta->bth',g.view(g.shape[0],g.shape[1],-1,self.n_control),u[label == 2])
            descent_value = torch.einsum('bth,bth->bt', d_b_transit, (f + gu))
            loss_4 = F.relu(self.eps_decent + descent_value.unsqueeze(-1) - b_transit).sum(dim=-1).mean()/(1e-5 + N_safe)
            return self.w_safe*loss_1, self.w_unsafe*loss_2, self.w_grad*loss_3, self.w_decent*loss_4, b_safe.mean(), b_unsafe.mean(), (ascent_value.unsqueeze(-1) + b_safe).mean(), (descent_value.unsqueeze(-1) + b_transit).mean()
        else:
            return self.w_safe*loss_1, self.w_unsafe*loss_2, self.w_grad*loss_3, 0*loss_3, b_safe.mean(), b_unsafe.mean(), (ascent_value.unsqueeze(-1) + b_safe).mean(), 0
...
class InDCBFTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_safe, loss_unsafe, loss_grad, loss_decent, b_safe, b_unsafe, b_acsent, b_decsent = self.barrier.loss_function(x,label,u,self.model.ode)
        train_loss['loss_safe'] = loss_safe
        train_loss['loss_unsafe'] = loss_unsafe
        train_loss['loss_grad'] = loss_grad
        train_loss['loss_decent'] = loss_decent
        train_loss['loss'] = train_loss['loss_latent']*self.w_latent \
                        +loss_safe*self.w_barrier+loss_unsafe*self.w_barrier+loss_grad*self.w_barrier+loss_decent*self.w_barrier
        self.log_dict({key: val.item() for key, val in train_loss.items()}, sync_dist=True)
        self.log_dict({'b_safe':b_safe,'b_unsafe':b_unsafe,'b_grad':b_acsent,'b_decsent':b_decsent},sync_dist=True)
        if batch_idx % 5 == 0:
            print()
            print(b_unsafe)
            print(b_safe)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        val_loss['loss_decent'] = loss_decent
        val_loss['loss'] = val_loss['loss_latent']*self.w_latent \
                        +loss_safe*self.w_barrier+loss_unsafe*self.w_barrier+loss_grad*self.w_barrier+loss_decent*self.w_barrier
        self.log_dict({f"val_{key}": val.item() for key, val in val_loss.items()}, sync_dist=True)
        self.log_dict({'val_b_safe':b_safe,'val_b_unsafe':b_unsafe,'val_b_grad':b_acsent,'val_b_decent':b_decsent},sync_dist=True)
    ...
    def on_train_epoch_start(self):
        if self.current_epoch == 10:
            self.barrier.w_safe = 1
            self.barrier.w_unsafe = 1
            self.barrier.w_grad = 1
            self.barrier.w_decent = 1
