...
class Barrier(torch.nn.Module):
    ...
    def loss_function(self,x,label,u,ode):
        ...
        b_bound = self.forward(x_bound)
        eps_safe = self.eps_safe*torch.ones_like(b_safe)
        eps_unsafe = self.eps_unsafe*torch.ones_like(b_unsafe)
        eps_bound = torch.zeros_like(b_bound)
        offset = -(self.eps_safe+self.eps_unsafe)/(b_bound.shape[1]-1)*torch.arange(b_bound.shape[1])+self.eps_safe
        eps_bound += offset.unsqueeze(-1).to(b_bound.device)
        ...
        if self.with_nonzero:
            ...
            if self.with_gradient:
                ...
                output['b_grad_descent'] = (descent_value.unsqueeze(-1) + b_bound).mean()
        if self.with_gradient:
            ...
            loss_5 = F.relu(self.eps_ascent - ascent_value.unsqueeze(-1) - b_safe).sum(dim=-1).mean()
            output['loss_grad_ascent'] = self.w_grad*loss_5
...
class InDCBFTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.train_barrier:
            ...
            train_loss['loss_bound'] = output['loss_bound']
            train_loss['loss'] += output['loss_safe']*self.w_barrier+output['loss_unsafe']*self.w_barrier+output['loss_bound']*self.w_barrier
            self.log_dict({'b_safe':output['b_safe'],'b_unsafe':output['b_unsafe'],'b_bound':output['b_bound']},sync_dist=True)
            if self.barrier.with_gradient:
                train_loss['loss_grad_ascent'] = output['loss_grad_ascent']
                train_loss['loss_grad_descent'] = output['loss_grad_descent']
                train_loss['loss'] += output['loss_grad_ascent']*self.w_barrier
                train_loss['loss'] += output['loss_grad_descent']*self.w_barrier
                self.log_dict({'b_grad_ascent':output['b_grad_ascent']},sync_dist=True)
                self.log_dict({'b_grad_descent':output['b_grad_descent']},sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.train_barrier:
            ...
            val_loss['loss_bound'] = output['loss_bound']
            val_loss['loss'] += output['loss_safe']*self.w_barrier+output['loss_unsafe']*self.w_barrier+output['loss_bound']*self.w_barrier
            self.log_dict({'val_b_safe':output['b_safe'],'val_b_unsafe':output['b_unsafe'],'val_b_bound':output['b_bound']},sync_dist=True)
            if self.barrier.with_gradient:
                val_loss['loss_grad'] = output['loss_grad']
                val_loss['loss'] += output['loss_grad']*self.w_barrier
                self.log_dict({'val_b_acsent':output['b_acsent']},sync_dist=True)
            if self.barrier.with_nonzero:
                val_loss['loss_nonzero'] = output['loss_nonzero']
                val_loss['loss'] += output['loss_nonzero']*self.w_barrier
                self.log_dict({'dB*g':output['dB*g']},sync_dist=True)
