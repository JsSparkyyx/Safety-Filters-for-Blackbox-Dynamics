...
class InDCBFBarrier(torch.nn.Module):
    ...
    def loss_function(self,x,label,u,ode):
        ...
        # x_g = torch.einsum("btn,btnh->btnh",weight,x_g).sum(2)
        b = self.cbf(x_g)
...
class SABLASBarrier(torch.nn.Module):
    ...
    #     loss_2 = F.relu(self.eps_unsafe+b_unsafe).sum(dim=-1).mean()
    #     output = {"loss_safe":self.w_safe*loss_1,"loss_unsafe":self.w_unsafe*loss_2,"b_safe":b_safe.mean(),"b_unsafe":b_unsafe.mean()}
    #     x_nom = x_safe + ode(x_safe,u[label == 0])*0.1
    #     def odefunc(t,state):
    #         f, g = dynamics(state)
    #         gu = torch.einsum("bthc,btc->bth",g.view(g.shape[0],g.shape[1],-1,self.n_control),u[label == 0])
    #         # b,t,h,c  b,t,c
    #         return f + gu
    #     timesteps = torch.Tensor([0,0.1]).to(x.device)
    #     x_tide = odeint(odefunc,x_safe,timesteps,rtol=5e-6)[1,:,:]
    #     x_nom = x_nom + (x_tide-x_nom).detach()
    #     ascent_value = b_safe + (self.forward(x_nom)-b_safe)*10
    #     loss_3 = F.relu(-ascent_value).sum(dim=-1).mean()
    #     output['loss_grad_ascent'] = self.w_grad*loss_3
    #     output['b_grad_ascent'] = ascent_value.mean()
    #     return output
